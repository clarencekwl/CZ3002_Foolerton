...
function CalendarPicker(element, options) {
    // Core variables.
    // By default this.date is set to today's but, due to this url thingy we set it to the date passed in
    // this.date = new Date();

    var url = window.location.pathname;
    var url_arr = url.split("/");
    var url_date = url_arr.pop();
    // console.log(`URL Date: ${url_date}`);
    var temp_date = new Date(url_date);
    temp_date.setHours(0, 0, 0);
    this.date = temp_date;

    // console.log(`Processed Date: ${this.date}`);
    // this._formatDateToInit(this.date);
    

    this.day = this.date.getDay();
    // console.log(`Today's Day: ${this.day}`);
...
CalendarPicker.prototype._insertNavigationButtons = function () {
    ...
    var arrow_src = document.getElementById("calendar_container");
    // console.log(arrow_src);
    var arrow_src = arrow_src.classList[0];
    // console.log(arrow_src);
    this.previousMonthArrow.innerHTML = `<img src="${arrow_src}" width=10px />`;
    this.nextMonthArrow.innerHTML = `<img src="${arrow_src}" width=10px />`;
...
CalendarPicker.prototype._insertDaysIntoGrid = function () {
    ...
    var days_prev = firstDayOfMonth - 1;
    // no. of days for next month to put in
    var days_next = 42 - (firstDayOfMonth) - arrayOfDays.length + 1;
    ...
    arrayOfDays.forEach(date => {
        ...
        // New implementation, selector starts at 
        var dateIsTheCurrentValue = this.value.toISOString().split("T")[0] === date.toISOString().split("T")[0];
        // console.log(this.value.toISOString().split("T")[0]);
        // console.log(date.toISOString().split("T")[0]);
        // console.log("\n")
        
        if (dateIsTheCurrentValue) this.activeDateElement = dateElement;
        // Old implementation, assuming that each time selector will start at today's date
...
var url_arr = url.split("/");
var url_date = url_arr.pop();
var temp_date = new Date(Date(url_date));

const todayDate = new Date();
currentToDateString.textContent = temp_date.toISOString().split("T")[0] == todayDate.toISOString().split("T")[0]?"Today":temp_date.toISOString().split("T")[0];
...
myCalender.onValueChange((currentValue) => {
    ...
    var url_date = `${currentValue.getFullYear()}-${currentValue.getMonth()+1}-${currentValue.getDate()}`
    // var url_date = currentValue.toISOString().split("T")[0];
    // console.log(url_date);

    var url = window.location.pathname;
    // console.log(url)
    
    var url_arr = url.split("/"); // Removes the date
    url_arr.pop();
    url = url_arr.join("/");
    url += "/" + url_date;
    // console.log(url);
